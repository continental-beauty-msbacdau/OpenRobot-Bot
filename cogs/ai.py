...
class AI_slash(slash_utils.ApplicationCog):
    def get_ai_text(self):
        ai_text = """The following is a conversation with an AI assistant. The assistant is helpful, creative, clever, intelligent and very friendly.
...
1.
    ...
    @slash_utils.user_command(name="nsfw-check")
    @describe(image="The image. This can be a URL or a image attached.")
    async def nsfw_check(
        self,
        ctx: slash_utils.Context,
        image=typing.Union[str, discord.Member, int],
    ):
        """
        NSFW Checks an Image.
...
**Labels:**{newline + label_str if label_str else ' None.'}
    ...
    @slash_utils.user_command(name="ocr")
    @describe(image="The image. This can be a URL or a image attached.")
    async def ocr(
        self,
        ctx: slash_utils.Context,
        image=typing.Union[discord.Member, str, int],
    ):
        """
        Optical Character Recognition. Reads text from images.
